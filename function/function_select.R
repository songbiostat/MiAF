...
## Select the associated taxon when combining lower and upper 1-sided AF test (unweighted and weighted)
select.l.u <- function(lowerlist, upperlist, com, label.u, label.w, weight = TRUE){
  loc.lower <- c()
  loc.upper <- c()
  if(1 %in% com) loc.lower <- lowerlist
  if(2 %in% com) loc.upper <- upperlist
  if(weight){
    sel.lower <- label.w[loc.lower]
    sel.upper <- label.w[loc.upper]
  }else{
    sel.lower <- label.u[loc.lower]
    sel.upper <- label.u[loc.upper]     
  }
  return(list(lower = sel.lower, upper = sel.upper))
}

## Select the associated taxon when combining unweighted and weighted AF test
select.u.w <- function(sel.unweight, sel.weight, com){
  sel.lower <- c()
  sel.upper <- c()
  if(1 %in% com){
...
## return values: P-values from MiAFu (unweighted), MiAFw (weighted), MiAF (combined) 
##                and selected associated taxon from lower and upper 1-sided P-values

MiAF <- function (Y, X, tree, cov = NULL, model = c("gaussian","binomial"), n.perm = 1000, seed = NULL)
{
  model <- match.arg(model)
  GuniF.cum <- GUniFrac_cum(X, tree)
  cum <- GuniF.cum$cum     # "extended" OTU table 
  cum.labels = GuniF.cum$cum.labels     # labels for "extended" OTU table
  ...
  p.u.w <- perm.mar.cov(Y, xx, binary = (model == "binomial"), cov = cov, res = TRUE, nperm = n.perm, seed = NULL)
  w2 <- p.u.w$weight
  ...
  sel.u <- select.l.u(sel.u.l, sel.u.u, com = outcome.u, label.u, label.w, weight = FALSE)
  ...
  sel.w <- select.l.u(sel.w.l, sel.w.u, com = outcome.w, label.u, label.w, weight = TRUE)
  
  AFcom <- combine.method(c(cur.UniFrac$pvalue, cur.wUniFrac$pvalue), rbind(cur.UniFrac$p.perm, cur.wUniFrac$p.perm), method = "AF")     # combine unweighted and weighted versions of P-values
  p.com <- AFcom$pvalue
  outcome.com <- AFcom$which.selected
  sel <- select.u.w(sel.u, sel.w, outcome.com)
  
  return(list(UniFrac = AF.UniFrac, wUniFrac = AF.wUniFrac, com = p.com, select = sel))
}
