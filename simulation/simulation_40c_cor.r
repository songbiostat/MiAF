## This is a simulation example for the correlated case of scenario 3.
## Under scenario 3 where OTUs are divided into 40 clusters, and the outcome is truly associated with the 1st cluster.
...
clu <- 40     ## OTUs are divided into 40 clusters under scenario 3
...
for (i in 1:rep.pow){
  ...
  Z2 <- rnorm(n, 0, 1) + scale(apply(X.inf, 1, sum), center = TRUE, scale = TRUE)
  Z <- cbind (Z1, Z2)
